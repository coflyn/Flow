...
- **Recently Played Online History**: Automatically persists played online tracks to local storage (`yt_online_history`). Switching to the Online tab or clearing the search query displays **Recently Played** tracks (🕒) for instant 1-tap playback, seamlessly falling back to **Trending & Recommended** (🔥) when history is empty.
- **Smart Title & Artist Reversal Parser**: Enhanced RegExp parser (`cleanYouTubeTitleAndArtist`) that checks author/channel names against video titles to dynamically distinguish `"Song Title - Artist"` vs `"Artist - Song Title"` patterns, eliminating swapped title/artist metadata listings while stripping clutter tags (`(Official Music Video)`, `[Official Audio]`, `(Lyrics)`, `HD`, `4K`, `(MV)`).
- **Sleek "Online" Cloud Pill Toggle**: Redesigned search source toggle selector (`_buildSearchSourceSegmentedToggle`) with an **`Online`** cloud check icon (`Icons.cloud_done_rounded`) and rounded pill border outline, complementing the `Local` device audio mode.
- **Instant Audio Transition & Crackle-Free Buffer Reset**: Pauses current audio instantly (`0ms`) and resets ExoPlayer audio track hardware buffers cleanly upon selecting a new track, eliminating audio overlap and volume stepping crackle during track switches.
- **HD 1:1 Square Album Artwork & Official Album Metadata**: Integrated async official iTunes metadata lookup (`fetchOfficialSongArtworkAndAlbum`) to resolve crisp **600x600 px** 1:1 square album artwork and official album names (e.g., `"Parachutes"`, `"1989"`) instead of widescreen letterboxed 16:9 video thumbnails or `"YouTube Music"` fallbacks.
- **Strict Deduplication & Noise Filtering Engine**: Track uniqueness interceptor (`seenKeys`) that deduplicates search results by `${cleanedArtist} - ${cleanedTitle}`, skipping duplicate re-uploads from fan/lyric channels. Filters out 10-hour loops, slowed/reverb edits, karaoke tracks, PMV animations, and reaction videos.
...
- **Remove from Recently Played & Cache Purger**: Added `removeOnlineTrackFromHistory` option to track options menu. Deletes selected tracks from `yt_online_history` and automatically purges associated temporary audio cache files (`yt_videoId.m4a`) from storage and memory.
- **Streamlined Online Track Options**: Cleaned up track options bottom sheet for online tracks by hiding irrelevant local file operations (*Ringtone Cutter, Go to Album/Artist, Edit Metadata, Hide from Library, Delete from Device*), leaving a compact, high-relevance action list.
