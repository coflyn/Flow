...
- **Dedicated Online Playlists Tab & Trending Community Playlists**: Added a dedicated Online Playlists tab displaying top YouTube Music community playlists (*Billboard Top 100*, *Pop Music Hits*, *Top Songs*, etc.) with up to 300 tracks per playlist (`limit=300`), horizontal scrolling carousel of 140x140px square artwork cards, Create Playlist, Import Playlist URL, and Online Smart Playlists (*Favourites*, *Last Played*, *Most Played*).
- **InnerTube WEB_REMIX Playlist Import Engine**: Integrated YouTube Music InnerTube WEB_REMIX `browseId: 'VL$plId'` playlist scraper (`fetchOnlinePlaylistDetails`). Correctly extracts original YouTube playlist titles (e.g. *"galau inggris"*), real track durations via `_parseDurationStringToMs` (fixing `0 sec` header duration & `00:00` track item bugs), and 100% real track lists, automatically saving imported playlists under **`My Playlists`** persistently in local storage across app restarts.
- **Isolated Playlist Playback Queue**: Fixed queue overwrite bug in `_playTrack` where playing an online track from a custom or imported playlist erroneously replaced `_playbackQueue` with Recently Played. Playing from a playlist now preserves the full playlist queue and Up Next sequence.
- **Renamed Online Last Played to Recently Played**: Updated online smart playlist label from `'Last Played'` to `'Recently Played'` across the Online Playlists tab and detail views for clear terminology.
- **Glassmorphic 2-Column Action Cards**: Redesigned Create & Import Playlist buttons into sleek 2-column glassmorphic gradient action cards (*New Playlist* & *Import URL*) with vibrant accent borders, circular icon badges, and tap feedback.
...
- **Persistent Online Favourites & Most Played Engine (`yt_online_favorites`)**: Liking online tracks (❤️) persistently saves them to local storage under `yt_online_favorites` across app launches. The Online *Most Played* smart playlist ranks online tracks strictly by play count frequency (`_playCounts` descending).
...
- **Strict 4-Cover Playlist Artwork Threshold**: Enforced a strict 4-track threshold (`tracks.length >= 4`) for stacked artwork collages in playlist cards. Playlists with fewer than 4 tracks maintain clean fallback circle icons (*Fire icon for Most Played*, *History icon for Last Played*, etc.).
...
- **Streamlined Online Track Options**: Cleaned up track options bottom sheet for online tracks by hiding irrelevant local file operations (_Ringtone Cutter, Edit Metadata, Hide from Library, Delete from Device_), leaving a compact, high-relevance action list.
