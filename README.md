...
- **Multi-Key Online Playlist Caching & Download All Tracks**: Mapped `_onlinePlaylistTracks` and `_userOnlinePlaylists` by `playlistId`, `playlistTitle`, and `finalTitle` for 0ms instant detail view opening without loading spinners. Added **Download All Tracks** option to playlist context menus to download all online tracks in a playlist for offline playback.
